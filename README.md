...
**Status: v2.0.2** — deploy simplification release (online-only install, fewer scripts). See
...
tar -xzf notebooklm-bridge-v2.0.2.tar.gz
cd notebooklm-bridge-v2.0.2
...
```bash
scripts/pack.sh
# → dist/notebooklm-bridge-v2.0.2.tar.gz  (+ .sha256 sidecar)
```
...
```bash
scp dist/notebooklm-bridge-v2.0.2.tar.gz user@bridge-host:~/
scp dist/notebooklm-bridge-v2.0.2.tar.gz.sha256 user@bridge-host:~/
ssh user@bridge-host 'sha256sum -c notebooklm-bridge-v2.0.2.tar.gz.sha256'
```
