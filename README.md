...
flowchart LR
    ...
    FE -- "/api/* + X-User-Id<br/>+ X-Shared-Secret" --> BE[FastAPI :8002<br/>uvicorn --workers 1]
    BE --> A[auth check]
...
| Auth | `backend/auth.py` | `X-User-Id` + `X-Shared-Secret` header validation |
...
- **Strict secret hygiene** — `secrets/` and `.env` are gitignored and
  excluded from the release tarball; operator places `auth.json`
  (`chmod 0600`) and a fresh `INTERNAL_AUTH_SHARED_SECRET` on each target.
...
| `INTERNAL_AUTH_SHARED_SECRET` | *(required)* | Shared HMAC secret between frontend bundle and backend. Never check in. |
...
| `X-Shared-Secret` | Must match `INTERNAL_AUTH_SHARED_SECRET` in `.env`. |
