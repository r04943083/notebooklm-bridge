...
**Status: v2.0.4** — deploy simplification release (online-only install, fewer scripts). See
...
tar -xzf notebooklm-bridge-v2.0.4.tar.gz
cd notebooklm-bridge-v2.0.4
...
```bash
scripts/pack.sh
# → dist/notebooklm-bridge-v2.0.4.tar.gz  (+ .sha256 sidecar)
```
...
```bash
scp dist/notebooklm-bridge-v2.0.4.tar.gz user@bridge-host:~/
scp dist/notebooklm-bridge-v2.0.4.tar.gz.sha256 user@bridge-host:~/
ssh user@bridge-host 'sha256sum -c notebooklm-bridge-v2.0.4.tar.gz.sha256'
```
