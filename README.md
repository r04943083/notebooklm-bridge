...
**Status: v1.0.7** — first internal-ready release. See
...
tar -xzf notebooklm-bridge-v1.0.7.tar.gz
cd notebooklm-bridge-v1.0.7
...
```bash
scripts/pack.sh
# → dist/notebooklm-bridge-v1.0.7.tar.gz  (+ .sha256 sidecar)
```
...
```bash
scp dist/notebooklm-bridge-v1.0.7.tar.gz user@bridge-host:~/
scp dist/notebooklm-bridge-v1.0.7.tar.gz.sha256 user@bridge-host:~/
ssh user@bridge-host 'sha256sum -c notebooklm-bridge-v1.0.7.tar.gz.sha256'
```
