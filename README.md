...
**Status: v2.0.1** — deploy simplification release (online-only install, fewer scripts). See
...
**TL;DR — four shell commands on the deploy host.** Required:

- Linux with a desktop environment (GNOME / KDE / XFCE / …)
- **Python ≥ 3.11** (any minor — 3.11 / 3.12 / 3.13 all work; pip picks the
  right wheels for whichever the host has)
- **Node ≥ 18** (only used to serve the prebuilt frontend)
- **Outbound HTTPS** to `pypi.org` (for `deploy.sh`), `notebooklm.google.com`
  (for the bridge), and `cdn.playwright.dev` (first-run Chromium download)

> **Multiple Python versions on the host?** `deploy.sh` auto-picks the first
> Python ≥ 3.11 it finds (probe order: `python3.13 → python3.12 → python3.11
> → python3`). If your Python lives outside `$PATH`, override with
> `PYTHON_BIN=/full/path/to/python3 bash deploy.sh`. See
> [`scripts/README_DEPLOY.md`](scripts/README_DEPLOY.md) §1.

```bash
# 1. Unpack the release tarball
tar -xzf notebooklm-bridge-v2.0.1.tar.gz
cd notebooklm-bridge-v2.0.1
...
#    same command for first deploy AND upgrade. Override the install path
#    with NOTEBOOKLM_BRIDGE_HOME=/path bash deploy.sh
bash deploy.sh

# 3. Switch to the install path (everything from here on runs there)
cd ~/notebooklm-bridge

# 4. Sign in to NotebookLM with YOUR Google account
#    (pops a real Chromium window — sign in, open the target notebook once,
#     then close the browser. Skip on upgrade if cookies still valid.)
...
```bash
scripts/pack.sh
# → dist/notebooklm-bridge-v2.0.1.tar.gz  (+ .sha256 sidecar)
```

The tarball bundles: backend `.py` sources, the pre-built frontend `dist/`,
the daemon scripts (`start-web` / `stop-web` / `status-web` / `_supervise` /
`login`), `deploy.sh`, and `README_DEPLOY.md`. Size: ~5MB (down from ~40MB
in v1.0.x — v2.0 dropped the offline `wheels/` and installs from PyPI online).

It does **not** bundle `secrets/`, `.env`, or `state.json` — cookies get
minted on the target host by `scripts/login.sh`.

### Transferring to the bridge host

```bash
scp dist/notebooklm-bridge-v2.0.1.tar.gz user@bridge-host:~/
scp dist/notebooklm-bridge-v2.0.1.tar.gz.sha256 user@bridge-host:~/
ssh user@bridge-host 'sha256sum -c notebooklm-bridge-v2.0.1.tar.gz.sha256'
```
...
Same command as first deploy — `deploy.sh` installs to the fixed path
(`~/notebooklm-bridge` by default), and `secrets/auth.json`, `.env`,
`state.json`, `.venv/` all stay in place across versions.

```bash
# On the bridge host:
tar -xzf notebooklm-bridge-vNEW.tar.gz
cd notebooklm-bridge-vNEW
bash deploy.sh                              # rsync source into ~/notebooklm-bridge, preserves runtime state
cd ~/notebooklm-bridge
bash scripts/stop-web.sh && bash scripts/start-web.sh
```

No re-login required (cookies in `secrets/auth.json` carry over) and
`state.json` is preserved so sessions survive the upgrade. v2.0 removed
the dedicated `update.sh` script — `deploy.sh` itself handles upgrades.
...
- Multiple Python versions on the host: set `PYTHON_BIN=/path/to/python3` before `deploy.sh`. Any Python ≥ 3.11 works. See `scripts/README_DEPLOY.md` §1.
...
```bash
# 1. Create a venv (any Python ≥ 3.11) and install backend with dev + runtime extras
python3 -m venv .venv
...
# 4. Playwright Chromium (needed for scripts/login.sh)
.venv/bin/playwright install chromium

# 5. Sign in to NotebookLM (same flow as IT — writes secrets/auth.json)
bash scripts/login.sh
```
...
Day-to-day dev only needs `start-web.sh` / `stop-web.sh` / `pytest` /
`npm run build`. Without `notebooklm-py` installed the bridge runs in
degraded mode — chat returns 503 but everything else still works, which
is useful for frontend-only iteration.
...
├── scripts/
│   ├── start-web.sh / stop-web.sh / status-web.sh / _supervise.sh   (daemon mgmt)
│   ├── login.sh                                                      (mint Google cookies)
│   ├── pack.sh                                                       (build release tarball)
│   ├── deploy.sh                                                     (run on target host)
