...
#   6. seed a random INTERNAL_AUTH_SHARED_SECRET into .env (only if still placeholder)
#   7. mirror the secret into frontend/.env.local for Vite
#   8. smoke-test: `notebooklm list` (proves the cookies actually work)
#
...
say "Step 1/8 — login (storage_state.json)"
...
say "Step 2/8 — detect notebooklm-py version"
...
say "Step 3/8 — pin pyproject.toml"
...
say "Step 4/8 — pip install -e '.[runtime]'"
...
# ── Step 5: .env (backend) ─────────────────────────────────
say "Step 5/8 — .env (INTERNAL_AUTH_SHARED_SECRET)"
...
if grep -q "<32B random" "$PROJECT_ROOT/.env"; then
    SECRET=$(openssl rand -hex 32)
    sed -i "s|<32B random — replace this placeholder>|$SECRET|" "$PROJECT_ROOT/.env"
    ok "Generated INTERNAL_AUTH_SHARED_SECRET (32 random bytes) and wrote to .env"
else
    SECRET=$(grep '^INTERNAL_AUTH_SHARED_SECRET=' "$PROJECT_ROOT/.env" | cut -d= -f2-)
    ok "INTERNAL_AUTH_SHARED_SECRET already set; reusing existing value"
fi

# ── Step 6: frontend/.env.local (Vite) ─────────────────────
say "Step 6/8 — frontend/.env.local"
FRONTEND_ENV="$PROJECT_ROOT/frontend/.env.local"
if [ ! -f "$FRONTEND_ENV" ] || ! grep -q "^VITE_SHARED_SECRET=$SECRET$" "$FRONTEND_ENV"; then
    {
        echo "# Auto-generated by scripts/setup.sh — mirror of INTERNAL_AUTH_SHARED_SECRET in ../.env"
        echo "# This file is gitignored. Re-run scripts/setup.sh to rotate."
        echo "VITE_SHARED_SECRET=$SECRET"
    } > "$FRONTEND_ENV"
    ok "Wrote $FRONTEND_ENV"
else
    ok "frontend/.env.local already mirrors the secret"
fi

if ! grep -qF "frontend/.env.local" "$PROJECT_ROOT/.gitignore" 2>/dev/null; then
    echo "frontend/.env.local" >> "$PROJECT_ROOT/.gitignore"
    ok "Added frontend/.env.local to .gitignore"
fi

# ── Step 7: CLI smoke test ─────────────────────────────────
say "Step 7/8 — CLI smoke test"
