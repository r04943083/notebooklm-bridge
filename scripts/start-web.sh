...
# Load .python-env first (if present) so .venv/bin/python — which doesn't
# carry rpath to libpython.so / libssl.so for self-built interpreters — can
# find its shared libs at runtime via LD_LIBRARY_PATH. Generated by
# scripts/check-python.sh; absent on hosts using distro-managed Python.
if [ -f "$PROJECT_ROOT/.python-env" ]; then
    # shellcheck disable=SC1091
    source "$PROJECT_ROOT/.python-env"
fi
